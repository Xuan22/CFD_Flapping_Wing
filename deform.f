...
C*******************************************************************************
     ...
     &                             zx0,zy0,zz0,zt0,init)
C*******************************************************************************
C....03/20/2018 Modified by Xuan to direct read in kinematics
      ...
      if (init.eq.0) then
        do j = 1,jmax
          do k = 1,kmax
            ...
            zy0(j,k) = zy(j,k,1)
            zz0(j,k) = zz(j,k,1)
            zt0(j,k) = -ug(j,k,1)*zx(j,k,1)-vg(j,k,1)*zy(j,k,1)   !what is zt0?/
     &                 -wg(j,k,1)*zz(j,k,1)
          enddo
        enddo
      else
      ...
      enddo
      enddo
********* added by Camli Badrya 7/30 ***************
********* define the relaxtion factor d ***********
** need to define: d and azm1
** phiMax_old
** phiOff_old
** beta_Old
      ...
      !!! define ismth to switch the relaxation 
      IF (ismth.GT.0) THEN
...
C... define the parameters for the second wing rotation 
...
C... done
      ...
      if (init.eq.1) then
         ...
         !dphi=dt1*2*pi*kin_freq*kin_phiMax*sin(2*pi*kin_freq*totime1 
c		 dphi=dt*rf*kin_phiMax*sin(abs(rf)*totime 
c     &        + kin_phi_phase*deg) + ro*delta_phiOff
		 temp=rf*totime+kin_phi_phase*deg
		 dphi=dt*kin_phiMax*rf*sin(temp) +ro*delta_phiOff
      ...
      if (init.eq.1) then
         dalpha=ro*alpha_g !kin_alphaOff ! set grid to initial pitch position
         kin_alpha=kin_alphaInit
      else
         !1st order derivative of kinematics equation:
         !temp=2*pi*kin_freq*totime1+kin_alpha_phase*deg
         !dalpha=-dt1*2*pi*kin_freq*kin_alphaMax*2.7*cos(temp)
c         temp=rf*totime+kin_alpha_phase*deg
c         dalpha=-dt*abs(rf)*(kin_alphaMax)*2.7*cos(temp)
c     &            /cosh(2.7*sin(temp))**2 - ro*delta_beta
         temp=rf*totime+kin_alpha_phase*deg
		 dalpha=-13.58*rf*sin(temp)*dt -12.5*rf*cos(temp)*dt -ro*delta_beta
         kin_alpha=kin_alpha+dalpha
      endif

      write(STDOUT,101) kin_phi,kin_alpha,kin_beta,dt,totime,azm,d
      write(STDOUT,102) delta_phiOff, delta_beta
...
102   format(/,' Flapping / pitching wing: delta_phiOff = ',f10.5,', delta_beta = ',f10.5)
      contains
         !----------------------------------------------------------------
     ...
     &            + p1(3)
      ...
      end subroutine flap_pitch_wing_md


C*******************************************************************************
      subroutine flap_pitch_wing(x,y,z,xg,yg,zg,ug,vg,wg,zx,zy,zz,
     &                             zx0,zy0,zz0,zt0,init)
C*******************************************************************************
C....Added by Camli and Mathieu
C....7/7/2014 Camli add stroke plane beta to kinematics
