module dummy_mpi
!!! Provides a dummy mpi definition to allow free use of MPI calls
!!! even when the code is to be compiled without MPI support. The code
!!! makes heavy use of definitions found in mpif.h found on Linux
!!! 64-bit processor along with Intel Fortran Compiler. The actual
!!! values do not matter because they should never be active.

!!! This module only provides definitions for a limited subset of MPI
!!! calls. If you need additional definitions add them here and set
!!! them to either silently return success codes, or abort if the call
!!! doesn't make sense in sequential mode.
...
!        SAVE   /MPIPRIVC/
       ...
       private :: get_sizeof
contains
   function get_sizeof(datatype) result(size)
      !! Determine size in bytes of the given datatype
      !!
      !! Inputs:
      ...
      case (MPI_CHARACTER, MPI_BYTE)
         size=1
      case (MPI_INTEGER,MPI_INTEGER4)
         size=4
      case (MPI_REAL)
         size=4
      case (MPI_DOUBLE_PRECISION,MPI_REAL8)
         size=8
      case default
         call stop_execution("dummy_mpi::get_sizeof","Unknown datatype")
      end select
   end function get_sizeof

   !! Wrapper MPI Definitions begin here
   !!
   !! First the 'do-nothing' routines are written out, followed by
   !! routines that if called in sequential mode generate an error.
   ...
   subroutine mpi_bcast(buf,count,datatype,root,comm,error)
   ...
   subroutine mpi_buffer_attach(buf,size,error)
      character(len=1), dimension(:) :: buf
      integer :: size, error
   ...
   subroutine mpi_buffer_detach(buf,size,error)
      character(len=1), dimension(:) :: buf
      integer :: size, error
   ...
   subroutine mpi_bsend(buf,count,datatype,dest,tag,comm,error)
      ...
      call stop_execution("mpi_bsend","Cannot call MPI_BSEND in &
            &sequential mode")
   ...
   subroutine mpi_isend(buf,count,datatype,dest,tag,comm,request,error)
      ...
      call stop_execution("mpi_isend","Cannot call MPI_ISEND in &
            &sequential mode")
   ...
   subroutine mpi_recv(buf,count,datatype,source,tag,comm,status,error)
      ...
      call stop_execution("mpi_recv","Cannot call MPI_RECV in &
            &sequential mode")
   ...
   subroutine mpi_irecv(buf,count,datatype,source,tag,comm,request,error)
      ...
      call stop_execution("mpi_irecv","Cannot call MPI_IRECV in &
            &sequential mode")
   end subroutine mpi_irecv
      
#endif

end module dummy_mpi
